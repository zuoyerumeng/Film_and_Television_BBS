      ...
      <table class=guide cellspacing=0 cellpadding=0 width="100%" border=0>
        ...
        <tr>
          <td>系统设置首页 » ${txt }</td></tr></tbody></table><br>
			<form method="post" action="<%=path%>/forumsEdit.action">
				<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableborder">
					<tr class="header"><td colspan="3">${txt }</td></tr>
					<tr align="center">
						<td class="altbg1" width="15%">名称:</td>
						<td class="altbg2" width="85%"><input type="text" name="name" value="${forumInfo.name }" size="20" maxlength="50"></td>
					</tr>
							...
							<td class="altbg2" width="85%">
								<select name="fup">
									<c:forEach items="${requestScope.forumsList}" var="forums">
										<c:if test="${forums.fid==forumInfo.fup }">
										<option value="${forums.fid }" selected="selected">${forums.name }</option>
										</c:if>
										<c:if test="${forums.fid!=forumInfo.fup }">
				...
				<center>
					...
					<input type="hidden" name="type" value="${forumInfo.type }">
